...
func GetVolumeMounts(mountCerts bool, mountKeys bool, mountKubeconfig bool) []corev1.VolumeMount {
	volumeMounts := []corev1.VolumeMount{
		...
		{
			Name:      "test-operator-logs",
			MountPath: "/var/lib/horizon/external_files",
			ReadOnly:  false,
		},
		{
			Name:      "horizon-clouds-config",
			MountPath: "/var/lib/horizon/.config/openstack/clouds.yaml",
			SubPath:   "clouds.yaml",
			ReadOnly:  true,
		},
		{
		...
		{
			Name:      "horizon-config",
			MountPath: "/etc/horizon/horizon.conf",
	...
	if mountKubeconfig {
		kubeconfigMount := corev1.VolumeMount{
			Name:      "kubeconfig",
			MountPath: "/var/lib/horizon/.kube/config",
			SubPath:   "config",
			ReadOnly:  true,
		}
